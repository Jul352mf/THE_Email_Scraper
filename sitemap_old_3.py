...
Efficiency upgrade
------------------
* Standard sitemap fetch remains full — no timeouts thrown away.
* Sub‑sitemaps fetched in parallel to avoid serial delays on large indexes.
* Logging of discovery duration only when no standard sitemap is found.
* Robots.txt fallback deduplicates by canonical URL and respects domain.
...
class SitemapParser:
    ...
    def get_priority_urls(self, domain: str) -> Tuple[List[str], bool]:
        start = time.time()
        priority: List[str] = []
        # materialize discovered sitemaps
        sitemap_urls = list(self.discover_sitemaps(domain))
        used = bool(sitemap_urls)
        if not used:
            return priority, False        
        
        log.debug("Parallel‐fetching %d sitemaps via %d threads", 
          len(sitemap_urls), min(4, len(sitemap_urls)))
        
        def fetch_and_extract(sm_url: str):
            canon = canonicalise(sm_url)
            data = self._sitemap_cache.get(canon)
            if data is None:
                resp = http_client.safe_get(sm_url, retry_count=2)
                if not resp:
                    return []
                data = resp.content
                self._sitemap_cache[canon] = data
            try:
                return list(self.parse_sitemap(data))
        ...
        with ThreadPoolExecutor(max_workers=4) as pool:
            futures = {pool.submit(fetch_and_extract, url): url for url in sitemap_urls}
